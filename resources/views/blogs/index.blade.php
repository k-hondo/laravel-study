...
@section('title', config('page_titles.blogs') . '一覧')
    ...
    <section class="bg-gray-100 pt-2">
        <div class="container mx-auto">
            <p class="text-left px-4 pt-2 text-gray-400"><a href="{{ route('index') }}"
                    ...
                    class="px-2">&gt;</span>{{ config('page_titles.blogs') }}</p>
            <p class="text-center pt-10 text-2xl">{{ config('page_titles.blogs') }}</p>
            ...
            <div class="my-8 pb-4 border-b">
                ...
                <ul class="flex text-center pt-2">
                    @foreach ($categories as $category)
                        <li class="bg-gray-100 text-gray-400 py-1 px-3 mr-3"><a
                                href="{{ route('blogs.index', ['category' => $category->id]) }}">{{ $category->name }}</a>
                        </li>
                    @endforeach
                    @foreach ($cats as $cat)
                        <li class="bg-gray-100 text-gray-400 py-1 px-3 mr-3"><a
                                href="{{ route('blogs.index', ['cat' => $cat->id]) }}">＃{{ $cat->name }}</a></li>
                    @endforeach
                </ul>
            </div>
                    ...
                    <article class="w-full md:w-1/2 lg:w-1/3 p-3">
                        <div class="border rounded-lg overflow-hidden shadow">
                            <div class="relative h-52">
                                ...
                                <a href="{{ route('blogs.show', $blog) }}"><img class="w-full h-56 object-cover"
                                        src="{{ asset('storage/' . $blog->image) }}" alt=""></a>
                                <time
                                    class="block text-xs text-gray-500 text-right pt-1 pr-2">{{ $blog->updated_at->format('Y.m.d') }}</time>
                            </div>
                                ...
                                <div class="flex justify-between">
                                    <ul class="flex">
                                        @foreach ($blog->cats as $cat)
                                            <li class="bg-gray-100 text-gray-400 text-xs mr-2 py-1 px-2">
                                                ＃{{ $cat->name }}
                                            </li>
                                        @endforeach
                                    </ul>
