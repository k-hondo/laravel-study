    ...
    <section>
        <div class="mb-6 py-4 bg-white rounded">
            <div class="flex px-6 pb-4 border-b">
                <h2 class="text-xl font-bold">ねこちゃん一覧</h2>
                <div class="ml-auto">
                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr class="text-xs text-gray-500 text-left">
                            ...
                            <th class="font-medium">種類</th>
                            <th class="font-medium">性別</th>
                            <th class="font-medium">年齢</th>
                            ...
                            <tr @class(['text-sm', 'bg-gray-50' => $loop->odd])>
                                <td class="flex px-4 py-3 items-center">
                                    ...
                                    <p class="font-medium"><a
                                            href="{{ route('admin.cats.edit', ['cat' => $cat]) }}">{{ $cat->name }}</a>
                                    </p>
                                </td>
                                <td class="font-medium">{{ $cat->breed }}</td>
                                <td class="font-medium">{{ $cat->gender->label() }}</td>
                                <td class="font-medium">{{ $cat->age }}さい</td>
                                <td>{{ $cat->updated_at->format('Y.m.d H:i:s') }}</td>
                                <td>
                                    <div class="flex">
                                        {{-- 編集ボタン --}}
                                        <a class="mr-2" href="{{ route('admin.cats.edit', ['cat' => $cat]) }}">
                                        ...
                                        {{-- 削除ボタン --}}
                                        <form action="{{ route('admin.cats.destroy', ['cat' => $cat]) }}" method="post"
