            ...
            <div class="py-4 bg-white rounded">
                <form action="{{ route('admin.cats.update', ['cat' => $cat->id]) }}" method="post"
                    ...
                    <div class="flex px-6 pb-4 border-b">
                        <h3 class="text-xl font-bold">ねこちゃん編集</h3>
                        <div class="ml-auto">
                            <button type="submit"
                    ...
                    <div class="pt-4 px-6">
                        ...
                        <div class="mb-6">
                            ...
                            <input id="name" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded"
                                type="text" name="name" value="{{ old('name', $cat->name) }}">
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <input id="breed" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded"
                                type="text" name="breed" value="{{ old('breed', $cat->breed) }}">
                        </div>
                            ...
                            <div class="flex">
                                <select id="gender"
                                    ...
                                    @foreach ($genders as $value => $label)
                                        <option value="{{ $value }}" @selected(old('gender', $cat->gender?->value) == $value)>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-6">
                            ...
                            <input id="date_of_birth" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded"
                                type="date" name="date_of_birth" value="{{ old('date_of_birth', $cat->date_of_birth) }}">
                        </div>

                        <div class="mb-6">
                            <label class="block text-sm font-medium mb-2" for="image">画像</label>
                            <div class="flex items-end">
                                <img id="previewImage" src="{{ asset('storage/' . $cat->image) }}"
                                    data-noimage="{{ asset('storage/' . $cat->image) }}" alt=""
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="introduction" class="block w-full px-4 py-3 mb-2 text-sm bg-white border rounded" name="introduction"
                                rows="5">{{ old('introduction', $cat->introduction) }}</textarea>
                        </div>
                    </div>
                </form>
            </div>
